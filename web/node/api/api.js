...
router.post('/recipes/addRecipe', function (req, res) {
    if (!req.body.recipeName || !req.body.ingredients || !req.body.instructions || !req.body.imageData || !req.body.category || !req.body.categoryId || !req.body.authorId) {
        res.status(400).json({ message: "Missing required parameters" });
    } else {
        databaseSql.handleAddRecipe(req.body.recipeName, req.body.ingredients, req.body.instructions, req.body.imageData, req.body.category, req.body.categoryId, req.body.authorId)
            .then(ok => { res.status(200).json(ok); })
            .catch(err => { res.status(500).json({ message: err }); });
    }
});
